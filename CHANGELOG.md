...
- **Magic Dash as a Hudu AI Tools resource** (issue #38). `hudu_magic_dash` now exposes Magic Dash to the AI Agent, mirroring the regular node's operations: `get`, `getAll`, and `createOrUpdate` (a write op, gated behind *Allow Write Operations*). Reads honour `output_markdown`/`include_frontmatter` (adds a `markdown_content` field, optionally with a YAML citation block) and writes honour `content_format` (`html`|`markdown`, converted to HTML before saving) — matching the Articles wiring. `createOrUpdate` resolves the numeric `company_id` to the `company_name` the Magic Dash API keys items by, and enforces the same `content`/`content_link` and `icon`/`image_url` mutual exclusivity as the regular node.
